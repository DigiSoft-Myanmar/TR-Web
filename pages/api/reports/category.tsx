...
export default async function handler(
  ...
  try {
    ...
    if (isInternal(session) && authorize) {
      switch (req.method) {
        case "GET":
          let { startDate, endDate, sellerId, categoryId } = req.query;
          ...
          orgOrders = orgOrders.filter((z) =>
            isCompleted(z.sellerResponse, seller.id)
          );
          ...
          let totalUnitSold = orders
            .map((z) =>
              z.cartItems
                .filter((b: any) => b.sellerId === seller.id)
                .map((z: any) => z.quantity)
                .reduce((a, b) => a + b, 0)
            )
            .reduce((a, b) => a + b, 0);
          ...
          let totalAuctions = orders
            .map((z) =>
              z.cartItems
                .filter(
                  (b: any) => b.sellerId === seller.id && b.isAuction === true
                )
                .map((z: any) => z.quantity)
                .reduce((a, b) => a + b, 0)
            )
