...
import ManufactureModal from "@/components/modal/sideModal/TextModal";
import {
...
function Default() {
  ...
  const [condition, setCondition] = React.useState<any>(undefined);
  const [value, setValue] = useState<string>("");
  const { t }: any = useTranslation("common");
  const { data: parentData, refetch } = useQuery("conditionsData", () =>
            ...
            <IconButton
              ...
              onClick={(e) => {
                ...
                showConfirmationDialog(
                  ...
                  () => {
                    fetch(
                      `/api/products/conditions?id=${encodeURIComponent(
                        row.id
                      )}`,
                      {
                        method: "DELETE",
  ...
  return isInternal(session) &&
    ...
    <div>
      ...
      <div className="relative px-4 py-8 bg-white rounded-md shadow-md">
        <h3 className="mb-5 text-lg font-semibold">Conditions</h3>
      ...
      <TextModal
        ...
        data={condition}
        apiPath="/api/products/conditions"
        type="Tag"
        updateFn={() => {
          refetch();
        }}
