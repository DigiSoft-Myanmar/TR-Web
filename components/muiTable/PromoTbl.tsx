...
import { isBetween } from "@/util/verify";
...
const PromoTbl = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: "Brands",
      renderCell: ({ row }: any) =>
        row.isAllowAllBrand === true ? (
          <Typography typography={"caption"}>All Brands</Typography>
        ) : (
          <Tooltip
            title={
              <>
                {row.brands.map((e: any, index: number) => (
                  <Typography typography={"caption"} key={index}>
                    {e.brandName}
                  </Typography>
                ))}
              </>
          ...
          >
            <Typography typography={"caption"}>
              {row.brandIds.length + " Brands Only"}
            </Typography>
          </Tooltip>
        ),
    },
    {
      ...
      headerName: "Buyers",
      renderCell: ({ row }: any) =>
        row.isAllowAllBuyers === true ? (
          <Typography typography={"caption"}>All Buyers</Typography>
        ) : (
          <Tooltip
            title={
              <>
                {row.users.map((e: any, index: number) => (
                  <Typography typography={"caption"} key={index}>
                    {e.phoneNum} - {e.username}
                  </Typography>
                ))}
              </>
            }
          >
            <Typography typography={"caption"}>
              {row.userIds.length + " Buyers Only"}
            </Typography>
          </Tooltip>
        ),
    },
