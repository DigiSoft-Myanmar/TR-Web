...
import { StaffPermission } from "@/types/permissionTypes";
...
const StaffTbl = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 100,
      headerName: "Role",
      field: "userDefinedRole",
      renderCell: ({ row }: CellType) => (
        <Typography variant="body2">{row.userDefinedRole?.name}</Typography>
      ),
    },
      ...
      <>
        <Card>
          <div className="flex w-full flex-row flex-wrap items-center px-5 pt-5">
            ...
            <div className="flex flex-row items-center gap-3">
              <ExportCSVButton
                ...
                permission={StaffPermission.staffExportAllow}
              />
              <button
                ...
                onClick={() => router.push("/users/create?role=" + Role.Staff)}
              >
                <svg
          ...
          <div className="flex w-full flex-row flex-wrap items-center justify-between gap-3 p-5">
            ...
            <StatsCard
              label={"Active Staffs (" + new Date().getFullYear() + ")"}
              currentCount={
                data.filter(
                  (e: any) =>
