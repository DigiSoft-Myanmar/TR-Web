...
import HighlightAuctionInfo from "./HighlightAuctionInfo";
...
function AuctionHome() {
  ...
  /* const { data } = useSWR(
    "/api/marketplace/featured?type=" + ProductType.Auction,
    fetcher
  ); */
  const data = [];
  const [currentProd, setCurrentProd] = React.useState<any>();
      ...
      if (current) {
        setShowList(
          data.filter(
            (j: any) =>
              j.categories &&
              j.categories.includes(current._id) &&
              (currentProd ? j._id !== currentProd._id : j._id !== data[0]._id)
          )
        );
      } else {
        setShowList(
  ...
  }, [data, current, maxProd]);

  return data ? (
    <section className="bg-brandDark mt-10 flex flex-col relative px-10 md:px-20">
      <div className="px-4 py-16 mx-auto max-w-screen-2xl sm:px-6 lg:px-8">
        {currentProd ? (
          <HighlightAuctionInfo
            id={currentProd._id.toString()}
            type={currentProd.type}
            name={currentProd.name}
            image={currentProd.imgList[0]}
            nameMM={currentProd.nameMM}
            seller={currentProd.seller}
            currentBid={currentProd.currentBid}
            openingBid={currentProd.openingBid}
            startTime={currentProd.startTime}
            endTime={currentProd.endTime}
            productInfo={currentProd.productInfo}
            productInfoMM={currentProd.producyInfoMM}
          ...
          <HighlightAuctionInfo
            ...
            type={data[0].type}
            name={data[0].name}
            image={data[0].imgList[0]}
            nameMM={data[0].nameMM}
            seller={data[0].seller}
            currentBid={data[0].currentBid}
            openingBid={data[0].openingBid}
            startTime={data[0].startTime}
            endTime={data[0].endTime}
            productInfo={data[0].productInfo}
            productInfoMM={data[0].producyInfoMM}
            slug={data[0].slug}
          />
        )}
        {data &&
          data.filter((j: any) =>
            ...
            <div className="relative -top-10 lg:-top-36 flex flex-col space-y-5 max-w-[80vw]">
              <div className="flex text-white md:items-center md:flex-row flex-col items-start">
                ...
                <Link
                  href={
                    "/marketplace?type=" +
                    encodeURIComponent(ProductType.Auction)
                  }
                  className="text-sm font-semibold hover:underline cursor-pointer underline-offset-4 self-end mt-5 md:mt-0"
                >
                  {t("seeMore")}
                </Link>
              </div>
              <div className="flex space-x-5 overflow-x-auto max-w-[250px] sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl text-white scrollbar-hide">
                <div
                  className={`${
                    current === "" ? "font-semibold" : " hover:italic"
                  } cursor-pointer text-sm hover:font-semibold whitespace-nowrap`}
                  onClick={(e) => setCurrent(undefined)}
                >
                  <h3>{t("recentlyListed")}</h3>
                  {!current && (
                    <div className="w-1/2 h-1 rounded-full bg-white mt-2"></div>
                  )}
                </div>
                {categoriesData &&
                  categoriesData.map((elem: any, index: number) => (
                    <div
                      className={`${
                        current && current._id === elem._id
                          ? "font-semibold"
                          : "  hover:italic"
                      } cursor-pointer text-sm hover:font-semibold whitespace-nowrap`}
                      key={index}
                      onClick={(e) => setCurrent(elem)}
                    >
                      <h3>
                        {locale === "mm" &&
                        elem.nameMM &&
              ...
              {/* <div className="grid grid-cols-auto220 place-self-center md:place-self-auto gap-3">
                  {showList
                    .filter((j: any) =>
                      currentProd
                        ? j._id !== currentProd._id
                        : j._id !== data[0]._id
                    )
                    .slice(0, 15)
                    .map((e: any, index: number) => (
                      <AuctionProductInfo
                        ...
                        endTime={e.endTime}
                        productImg={"/api/files/" + encodeURIComponent(e.imgList[0])}
                        seller={e.seller}
                        type={e.type}
                        name={e.name}
                        openingBid={e.openingBid}
                        nameMM={e.nameMM}
                        slug={e.slug}
              ...
              {showList &&
              showList.filter((j: any) =>
                currentProd ? j._id !== currentProd._id : j._id !== data[0]._id
              ).length > 0 ? (
                <div className="w-full sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl overflow-hidden">
                  <Swiper
                      ...
                      .map((e: any, index: number) => (
                        ...
                        >
                          {/* <AuctionProductInfo
                            ...
                            endTime={e.endTime}
                            productImg={
                              "/api/files/" + encodeURIComponent(e.imgList[0])
                            }
                            seller={e.seller}
                            type={e.type}
                            name={e.name}
                  ...
                  <Swiper
                    ...
                    scrollbar={{ draggable: true }}
                    centeredSlides={true}
                    className="flex lg:hidden max-w-xs"
                  >
                    {showList
                      .filter((j: any) =>
                        currentProd
