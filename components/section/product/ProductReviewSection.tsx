...
function ProductReviewSection({ productId }: { productId: string }) {
  ...
  const { data: buyerReview, refetch } = useQuery(
    ["productReview", productId],
    () =>
      fetch("/api/products/reviews?id=" + encodeURIComponent(productId)).then(
        (res) => {
          let json = res.json();
          return json;
        }
  ...
  function submitReview(data: any) {
    fetch("/api/products/reviews?id=" + encodeURIComponent(productId), {
      method: "POST",
      body: JSON.stringify({
  ...
  return (
    <div
      className={`${
        isInternal(session) ? "py-5 flex flex-col gap-5" : "flex flex-col gap-5"
      }`}
    >
      {isInternal(session) === false && buyerReview?.canReview === true && (
        <form
          className="p-3 rounded-md flex flex-col gap-5"
      ...
      <div className="bg-white p-5 rounded-md flex flex-col gap-5">
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-5 justify-between">
          ...
          <Tooltip title="Ratings are provided by buyers based on experience with this product.">
