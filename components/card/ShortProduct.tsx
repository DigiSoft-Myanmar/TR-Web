...
function ShortProduct({ product }: { product: any }) {
          ...
          if (pricingInfo.maxSaleEndDate) {
            if (
              ...
              let remainingTime =
                new Date(pricingInfo.maxSaleEndDate!).valueOf() -
                new Date().valueOf();
              setRemainingTime(convertStr(convertMsToTime(remainingTime)));
            } else {
              setRemainingTime(undefined);
            }
          }
  ...
  return (
    <div
      className="relative flex cursor-pointer flex-row flex-wrap items-stretch space-x-3 rounded-md border bg-white shadow-md md:max-h-[250px] md:flex-nowrap"
      onClick={() => {
        router.push(
          accessKey
            ? "/marketplace/" +
    ...
    >
      <div className="relative w-full md:h-full">
        <Image
          ...
          className="max-h-[250px] w-full md:h-full md:max-h-full md:min-w-[200px]"
        />
      ...
      <div className="flex w-full flex-col space-y-3 p-5 md:min-w-[250px]">
